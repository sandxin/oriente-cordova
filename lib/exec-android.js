...
import nativeApiProvider from './nativeapiprovider';
...
function androidExec(success, fail, service, action, args) {
  ...
  for (var i = 0; i < args.length; i++) {
    if (utils.typeName(args[i]) == 'ArrayBuffer') {
      args[i] = fromArrayBuffer(args[i]);
    }
  }

  var callbackId = service + originCDV.callbackId++,
    argsJson = JSON.stringify(args);
  if (success || fail) {
    originCDV.callbacks[callbackId] = {
      success: success,
      fail: fail
    };
  }
...
function hookOnlineApis() {
  function proxyEvent(e) {
    originCDV.fireWindowEvent(e.type);
  }
  ...
  originCDV.addWindowEventHandler('online');
  originCDV.addWindowEventHandler('offline');
...
function buildPayload(payload, message) {
  ...
  if (payloadKind == 's') {
    ...
    payload.push(toArrayBuffer(data));
...
function processMessage(message) {
  ...
  if (firstChar == 'J') {
    ...
    originCDV.callbackFromNative(callbackId, success, status, payload, keepCallback);
  } else {
